...
extern "C" void * MYCDECL CUSTOM_MEMALIGN (size_t alignment, size_t size)
{
  ...
  if (0) { //  (size <= PAGE_SIZE) {
    // DieHard-specific: objects up to this size are naturally aligned
    // to powers of two.
    return CUSTOM_MALLOC (size);
  }
  // We align it ourselves.
  const size_t requestedSize = 2 * alignment + size;
  void * buf = CUSTOM_MALLOC (requestedSize);
  void * alignedAddr = (void *) (((size_t) buf + alignment - 1) & ~(alignment - 1));
  assert ((size_t) alignedAddr % alignment == 0);
  assert ((size_t) alignedAddr + sz < (size_t) buf + requestedSize);
  return alignedAddr;
}
...
extern "C" void CUSTOM_PREFIX(malloc_lock) (void) {
  getCustomHeap()->lock();
}

extern "C" void CUSTOM_PREFIX(malloc_unlock) (void) {
...
extern "C" int CUSTOM_PREFIX(posix_memalign) (void **memptr, size_t alignment, size_t size)
{
