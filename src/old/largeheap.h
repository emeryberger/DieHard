...
private:
  ...
  inline void set (void * ptr, size_t sz) {
    ...
    size_t currSize = sz;
    int iterations = (sz + LARGEHEAP_PAGE_SIZE - 1) / LARGEHEAP_PAGE_SIZE;
    for (int i = 0; i < iterations; i++) {
      _objectSize.set ((char *) ptr + i * LARGEHEAP_PAGE_SIZE, currSize);
      currSize -= LARGEHEAP_PAGE_SIZE;
    }
  }
  ...
  inline void clear (void * ptr) {
    ...
    int iterations = (sz + LARGEHEAP_PAGE_SIZE - 1) / LARGEHEAP_PAGE_SIZE;
    for (int i = 0; i < iterations; i++) {
      _objectSize.erase ((void *) ((char *) ptr + i * LARGEHEAP_PAGE_SIZE));
    }
  }

  enum { LARGEHEAP_PAGE_SIZE = PAGE_SIZE };
