...
extern "C" {
  ...
  void * MACWRAPPER_PREFIX(malloc) (size_t sz) {
    ...
    if (mallocs % 1000 == 0) {
      char buf[255];
      sprintf (buf, "%d %lf\n", mallocs, (float) sizes / (float) mallocs);
      fprintf (stderr, buf);
    }
#endif
    return xxmalloc(sz);
...
extern "C" {
  // operators new and delete, as mangled by g++.
  void * _Znwm ();
  void * _Znam ();
  void _ZdlPv ();
  void _ZdaPv ();
...
extern "C" {
  ...
  void MACWRAPPER_PREFIX(malloc_zone_batch_free)(malloc_zone_t *, void ** to_be_freed, unsigned num) {
    // FIX ME
    // for now, do nothing.
  }
  ...
  void MACWRAPPER_PREFIX(malloc_zone_print)(malloc_zone_t * zone, bool verbose) {
    // Do nothing.
  }
  ...
  void MACWRAPPER_PREFIX(malloc_set_zone_name)(malloc_zone_t *, const char * name) {
    // do nothing.
  }

  malloc_zone_t * MACWRAPPER_PREFIX(malloc_create_zone)(vm_size_t start_size,
						    unsigned flags)
  {
  ...
  malloc_zone_t * MACWRAPPER_PREFIX(malloc_zone_from_ptr) (const void * ptr) {
    return NULL;
  }
  
  void * MACWRAPPER_PREFIX(malloc_zone_malloc) (malloc_zone_t * zone, size_t size) {
    return MACWRAPPER_PREFIX(malloc) (size);
  }
  
  void * MACWRAPPER_PREFIX(malloc_zone_calloc) (malloc_zone_t * zone, size_t n, size_t size) {
    return MACWRAPPER_PREFIX(calloc) (n, size);
  }
  
  void * MACWRAPPER_PREFIX(malloc_zone_valloc) (malloc_zone_t * zone, size_t size) {
    return MACWRAPPER_PREFIX(valloc) (size);
  }
  
  void * MACWRAPPER_PREFIX(malloc_zone_realloc) (malloc_zone_t * zone, void * ptr, size_t size) {
    return MACWRAPPER_PREFIX(realloc) (ptr, size);
  }
  
  void * MACWRAPPER_PREFIX(malloc_zone_memalign) (malloc_zone_t * zone, size_t alignment, size_t size) {
    return MACWRAPPER_PREFIX(memalign) (alignment, size);
  }
  
  void MACWRAPPER_PREFIX(malloc_zone_free) (malloc_zone_t * zone, void * ptr) {
    xxfree (ptr);
  }
  
  void MACWRAPPER_PREFIX(malloc_zone_free_definite_size) (malloc_zone_t * z, void * ptr, size_t sz) {
    xxfree (ptr);
  }
...
other replacements to verify (from valgrind vg_replace_malloc.c):
...
FREE(VG_ZE,       _ZdlPv,               __builtin_delete );
...
// operator deletfined(VGP_ppc32_aix5) || defined(VGP_ppc64_aix5)
