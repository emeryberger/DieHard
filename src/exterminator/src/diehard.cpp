...
//  The process ID of the voter
int voterPid;

//  Process ID of main diehard executable (harness)
int harnessPid;
...
extern "C" void relayToVoter(int signum) {
  kill(voterPid,signum);
}
...
extern "C" void handleChildCrash (int v)
{
  ...
  sprintf(buf, "(%d, harness: %d) caught sig %d with status %d\n", getpid(), harnessPid, v, status);
  fputs(buf,stderr);
  ...
  //while (1) 
  {
    ...
    sprintf(buf, "(%d) caught sig %d on pid %d with status %d\n", getpid(), v, pid, status);
    fputs(buf,stderr);
  }

  if(pid == voterPid) {
    fprintf(stderr,"voter died\n");
    voterPid = 0;
  }
...
int main (int argc, char ** argv)
{
  ...
  // Send USR1 from child (notification of corruption detection) to voter
  signal (SIGUSR1, relayToVoter);
  ...
  for (int i = 0; i < NUMBER_OF_REPLICAS; i++) {
    ...
    if ((executablePid[i] = fork()) == 0) {
      ...
      if(i == 0) {
        sprintf (wholeString, 
                 "LD_PRELOAD=%s:/usr/lib/libdl.so DH_COREFILE=./DHcore-%d-%d DH_SEED=%d DH_REPLICATED=%d %s", 
                 "libdiefast.so", getppid(), i, i+2, getppid(), executeString);
      } else {
        sprintf (wholeString, 
                 "LD_PRELOAD=%s:/usr/lib/libdl.so DH_COREFILE=./DHcore-%d-%d DH_SEED=%d DH_REPLICATED=%d %s", 
                 "libdiefast.so", getppid(), i, i+2, getppid(), executeString);
