...
public:
  ...
  inline virtual void * malloc (size_t) { abort(); return NULL; }
  inline virtual bool free (void *) { abort(); }
  inline virtual size_t getSize (void *) { abort(); return 0; }
...
private:
  ...
  typedef OneHeap<BumpAlloc<PAGE_SIZE, MmapAlloc> > TheAllocator;

  // The type of a mini heap.
  template <unsigned long N> class MiniHeapType
    : public MiniHeap<Numerator, Denominator, ObjectSize, N, TheAllocator, DieFastOn> {};
